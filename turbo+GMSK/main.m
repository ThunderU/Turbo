...
% Value of SNR db
EbN0db = input(' Please enter Eb/N0 in dB : default [0:0.5:10] ');
if isempty(EbN0db)
    EbN0db = 0:0.5:10;
end
...
for nEN = 1:length(EbN0db)
    ...
    while nframe(nEN,1)<ferrlim
        ...
        for k = 1:5
            ...
            modulated = gmsk_mod(en_output);
            r = modulated+sigma*randn(1,length(modulated)); % received bits
            demodulated = gmsk_demod(r);
            yk = demultiplex(demodulated,alpha,puncture); % demultiplex to get input for decoder 1 and 2
